...
While AI within education does not have to be inherently bad at all time, more often then not, AI is promoting superficial learning. Rather than actually learning and excersing their brains, many students now rely on AI to provide an easy escape from the first signs of struggle. This overreliance has created a need for tools that encourage active thinking and critical engagement within educational settings.
...
4. **Depth analysis** - Open-ended answers are evaluated by a fine-tuned NLI (Natural Language Inference) model trained specifically to distinguish:
